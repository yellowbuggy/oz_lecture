...
function addBook() {
    ...
    `; // ! 삭제 버튼을 우측에 정렬하기 위해서 버튼들을 div로 묶음
...
function removeBook(button) {
    // li 요소와 제목 추출
    const li = button.parentElement.parentElement;  // ! 버튼을 div로 묶어서 부모 요소의 부모 요소가 li
    ...
    const isTitleInBooks = (book) => book.title === title;
    const titleIndexInBooks = books.findIndex(isTitleInBooks);
    books.splice(titleIndexInBooks, 1);

    // TODO(도전과제) : rentals 배열에서 대여 상태 제거 (findIndex, splice 사용)
    const isTitleInRentals = (rental) => rental.getStatus().title === title;
    const titleIndexInRentals = rentals.findIndex(isTitleInRentals);
    rentals.splice(titleIndexInRentals, 1);
...
function toggleRental(button) {
    ...
    const rental = rentals.find((rentalObj) => rentalObj.getStatus().title === title);
    if (!rental) return;
